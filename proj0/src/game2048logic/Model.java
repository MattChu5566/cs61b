...
public class Model {
    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int x = 0; x < size() - 1; x += 1) {
            for (int y = 0; y < size(); y += 1) {
                Tile tileLeft = board.tile(x, y);
                Tile tileRight = board.tile(x + 1, y);
                if (tileLeft.value() == tileRight.value()) {
                    return true;
                }
            }
        }
        for (int y = 0; y < size() - 1; y += 1) {
            for (int x = 0; x < size(); x += 1) {
                Tile tileBot = board.tile(x, y);
                Tile tileTop = board.tile(x, y + 1);
                if (tileBot.value() == tileTop.value()) {
                    return true;
                }
            }
        }
    ...
    int checkUpper(int x, int y, int myValue) {
        if (y == size() - 1) {
            return y;
        }
        int targetY = y + 1;
        ...
        if (targetTile == null) {
            return checkUpper(x, targetY, myValue);
        }
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        if (currTile != null) {
            ...
            int targetY = checkUpper(x, y, myValue);
            if (targetY > y) {
